...
struct Graph* createGraph(int V, int E)
{
    ...
    graph->V = V; // assigning values to structure elements taken from user
...
{
    //this function prints the finals solution
...
void BellmanFord(struct Graph* graph, int source)
{
    ...
    // This is the initial step that we know, we initialize all distance to infinity
    // except source. We assign source distance as 0
    ...
    //The shortest path of graph that contains V vertices (v-1 iterations)
    for(int i = 1; i < V-1; i++){
        for(int j = 0; j < E; j++){
